...
pub async fn run(args: PdbeSyncArgs, parent_args: &SyncArgs, ctx: AppContext) -> Result<()> {
    ...
    println!("Syncing PDBe-specific data from {}...", mirror.name);
    println!(
        "Data types: {}",
        args.data_types
            .iter()
    ...
    for data_type in &args.data_types {
        let (success, files, bytes) = sync_pdbe_data_type(
            *data_type,
            &dest,
...
/// Sync a single PDBe data type.
#[allow(clippy::too_many_arguments)]
async fn sync_pdbe_data_type(
    data_type: PdbeDataType,
    dest: &Path,
    subpath: Option<&str>,
    delete: bool,
    bwlimit: Option<u32>,
